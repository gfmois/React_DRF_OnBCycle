export default function ListTableComponent({ thead = [], items = [], itemClicked }) {
    return (
        items.length > 0
                        ...
                        {items.map((e, i) => {
                            return <tr className={i % 2 == 0 ? "bg-white border-b dark:bg-gray-900 dark:border-gray-700" : "border-b bg-gray-50 dark:bg-gray-800 dark:border-gray-700"} key={i}>
                                ...
                                <td className="px-6 py-4 flex flex-row gap-4">
                                    <div className="font-medium text-amber-600 dark:text-amber-500 hover:underline cursor-pointer" onClick={() => itemClicked(e)}>Edit</div>
